...
if __name__ == '__main__':
    ...
    exit()
    iteration = 0
    for ind in range(int(np.floor(len(digits_dattaset)/batch_size))):
        inputsTensor = torch.zeros(batch_size, 1, 100, 100)
        labelsTensor = torch.zeros(batch_size, 2)
        for i in range(batch_size):

            sample = digits_dattaset.__getitem__(ind+i)
            print('ind image = ', ind+i, 'sample = ', type(sample))
            ...
            # print('inputsTensor = ', inputsTensor.shape)
            # print('labelsTensor = ', labelsTensor.shape)

            # print('\n\ni = ', i, 'inputs = ', inputs.shape)
            inputsTensor[i, 0, :, :] = inputs
            labelsTensor[i, :] = labels

        print("inputsTensor Shape: ", inputsTensor.shape)
        print('labelsTensor Shape: ', labelsTensor.shape)

        optimizer.zero_grad()  # Clear off the gradient in (w = w - gradient)
        model.train()  # Put the network into training mode

        outputs = model(inputsTensor)
        loss = loss_fn(outputs, labelsTensor)
        exit()

    # iteration = 1
    exit()
    UnderstandOnTrail(digits_dattaset)
